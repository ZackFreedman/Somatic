...
class SomaticTrainerHomeWindow(Frame):
    ...
    def __init__(self, master=None):
        ...
        self.test_image = ImageTk.PhotoImage(image=_gesture_to_image([e[0] for e in quat_test_data], 50, 50, 2, 2, 2))

        for i in range(10):
            for j in range(5):
                self.thumbnail_frame.grid_columnconfigure(j, weight=1)
                button = Button(self.thumbnail_frame, image=self.test_image)
                button.grid(row=i, column=j)

        self.thumbnail_canvas.configure(scrollregion=(0, 0, 250, 500))
    ...
    def visualize(self, path):
        ...
        for x_coord, y_coord in _path_to_line_points(path, 250, 250, 5, 5):
